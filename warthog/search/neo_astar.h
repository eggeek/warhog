...
typedef jps_heuristic H;
template <class E>
class neo_astar 
{
	public:
		neo_astar(H* heuristic, E* expander)
	...
	private:
		...
		// no copy
		neo_astar(const neo_astar& other) { } 
		neo_astar& 
		operator=(const neo_astar& other) { return *this; }
      ...
      heuristic_->set_target(goalid);
			...
			start->set_g(0);
			start->set_f(heuristic_->h(startid) * hscale_);
			open_->push(start);
			...
			while(open_->size())
			{
					   	...
					   	expander_->next(n, cost_to_n))
				{
					...
					if(open_->contains(n))
					{
						...
						{
              n->set_g(gval);
						  n->set_f(gval + heuristic_->h(n->get_pdir(), n->get_id()) * hscale_);
...
template <class E>
uint32_t warthog::neo_astar<E>::searchid_ = 0;
