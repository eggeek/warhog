...
jps2_exp_prune::jps2_expansion_policy_prune(warthog::gridmap* map)
{
	map_ = map;
  ...
  jpruner.mapper = mapper;
	...
	jpl_ = new warthog::online_jump_point_locator2_prune(map, &jpruner, nodepool_);
	reset();
...
jps2_exp_prune::~jps2_expansion_policy_prune()
{
	delete jpl_;
	delete nodepool_;
  delete mapper;
